...
## Environment
The notebooks were re-executed with the package versions pinned in [`requirements.txt`](requirements.txt) (Python 3.11).
Run them from inside `plot_examples/`, because the data and law-weight paths are relative to that directory:

```bash
pip install -r requirements.txt
cd plot_examples
jupyter nbconvert --to notebook --execute --inplace scaling_law_fit_example.ipynb UR50_UniMeta_Leaning_curve_example.ipynb
```
